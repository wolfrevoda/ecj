...
public class Subpopulation implements Group
    ...
    /** The largest depth for the graph */
    public int maxDepth = 6;
    
    /** The smallest depth for the graph */
    ...
    */
    ...
    /**
     * Return max depth for the subIndividual set
     * @param subIndividual
     * @return
     */
    public int maxDepth(Individual[] subIndividual)
    {
    	int max = 0;
    	for(int i = 0; i < subIndividual.length; ++i)
    	{
    		GNPIndividual temp = (GNPIndividual)(subIndividual[i]);
    		for(int j = 0; j < temp.graphs.length; ++j)
    		{
    			int depth = temp.graphs[j].child.depth();
    			if(max < depth)
    				max = depth;
    		}
    	}
    ...
    /**
     * Produce populate for gnp algorithm
     * @param state
     * @param thread
     */
    public void gnpPopulate(EvolutionState state, int thread)
    {
    	int len = individuals.length;           // original length of individual array
        int start = 0;                                          // where to start filling new individuals in -- may get modified if we read some individuals in
        ...
        // populating the remainder with different hierarchy individuals
        HashMap h = null;
        if (numDuplicateRetries >= 1)
            h = new HashMap((individuals.length - start) / 2);  // seems reasonable
        ...
        prob = (SimpleProblemForm)(se.p_problem.clone());
        ...
        while(count < maxDepth)
        {
	        for(int x=start;x<individuals.length;x++) 
	        {
	            for(int tries=0; tries <= /* Yes, I see that*/ numDuplicateRetries; tries++)
	            {
	            	if(individuals[x] == null)
	            	{
	            		individuals[x] = species.newIndividual(state, thread, individuals, count);
	            		prob.evaluateGNP(state, individuals[x], 0, 0);
	            	}
	            	else
	            	{
	            		int worse = worseIndividual();
	            		Individual temp = species.newIndividual(state, thread, individuals, count);
	            		prob.evaluateGNP(state, temp, 0, 0);
	            		if(temp.fitness.betterThan(individuals[worse].fitness))
	            			individuals[worse] = temp;
    ...
    public void sort()
    {
    	/*非空节点个数*/
    	int elementNum = 0;
    	while(individuals[elementNum] != null && elementNum < individuals.length)
    		++elementNum;
    	int i = 0;
    	int j = elementNum - 1;
    	quickSort(i, j);
    ...
    public void quickSort(int first, int last)
    {
    	...
    	while(i < j)
    	{
    		while(individuals[j].fitness.betterThan(temp.fitness) && i < j)
    			--j;
    		individuals[i] = individuals[j];
    		while(temp.fitness.betterThan(individuals[i].fitness) && i < j)
    			++i;
    		individuals[j] = individuals[i];   				 
    	}
    	mid = i;
    	individuals[mid] = temp;
    	quickSort(first, mid - 1);
    	quickSort(mid + 1, last);
    }
    
    public int worseIndividual()
    {
    	int ID = 0;
    	Fitness smallID = individuals[0].fitness;
    	for(int i = 1; i < individuals.length; ++i)
    	{
    		if(individuals[i] == null)
    			break;
    		if(smallID.betterThan(individuals[i].fitness))
    		{
    			smallID = individuals[i].fitness;
    			ID = i;
    		}
    	}
